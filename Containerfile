...
# Note: Podman uses rootless containers by default
...
# Podman runs rootless by default — use UID 1000
# No need for explicit useradd in rootless Podman
# but keeping for Docker compatibility
RUN useradd -m -u 1000 gateway && \
    chown -R gateway:gateway /app /var/log/pii-gateway
USER gateway
